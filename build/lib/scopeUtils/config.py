...
class ConfigManager:
    ...
    def get_input_output_path(self, key):
        config = self.load_config()
        return Path(config['input_output_paths'][key]) if config else None
...
def get_input_output_path(key):
    return config_manager.get_input_output_path(key)
...
def main_function():
    ...
    from ..scopeUtils/config import get_input_output_path
    
    # Get paths from config
    data_dir = get_input_output_path('raw_data_dir')
